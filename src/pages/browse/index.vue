...
const TEXTS = { title:'\u6b3e\u5f0f\u4e2d\u5fc3', subtitle:'\u652f\u6301 COS \u4e0e\u897f\u670d\u53cc\u5206\u7c7b\u6d4f\u89c8\uff0c\u9ed8\u8ba4\u6309\u70ed\u5ea6\u4f18\u5148\u5c55\u793a\uff0c\u89d2\u8272\u957f\u56fe\u4f1a\u4ee5\u6574\u56fe\u4f18\u5148\u7684\u6d77\u62a5\u65b9\u5f0f\u5448\u73b0\u3002', cosLabel:'COS\u670d\u9970', suitLabel:'\u897f\u670d\u6b3e\u5f0f', searchPlaceholder:'\u641c\u7d22\u6b3e\u5f0f\u540d\u3001\u98ce\u683c\u6216\u9762\u6599', filterButton:'\u7b5b\u9009', currentLabel:'\u5f53\u524d\u5206\u7c7b\uff1a', resultLabel:'\u7ed3\u679c\u6570\u91cf\uff1a', coverAlt:'\u6b3e\u5f0f\u56fe', fallbackTitle:'\u672a\u547d\u540d\u6b3e\u5f0f', fallbackSubtitle:'\u98ce\u683c\u5f85\u5b8c\u5584', materialTag:'\u7cbe\u9009\u9762\u6599', hotPrefix:'\u70ed\u5ea6 ', cosBadge:'\u89d2\u8272\u6d77\u62a5', suitBadge:'\u897f\u670d\u7075\u611f', viewDetail:'\u67e5\u770b\u8be6\u60c5', viewInspiration:'\u67e5\u770b\u7075\u611f', emptyText:'\u6ca1\u6709\u627e\u5230\u5339\u914d\u6b3e\u5f0f\uff0c\u8bd5\u8bd5\u66f4\u77ed\u7684\u5173\u952e\u8bcd', needLogin:'\u67e5\u770b\u8be6\u60c5\u524d\u8bf7\u5148\u767b\u5f55\u6216\u6ce8\u518c', suitTip:'\u897f\u670d\u8be6\u60c5\u9875\u6b63\u5728\u5347\u7ea7\uff0c\u53ef\u5148\u67e5\u770b\u70ed\u95e8\u7075\u611f\u5217\u8868\u3002' }
export default {
  name: 'BrowsePage',
  components: { ProductPosterCard },
  data() { return { texts: TEXTS, baseUrl: this.$config.baseUrl, loading: false, activeType: 'cos', keyword: '', searchKey: '', allCosList: [], allSuitList: [] } },
  computed: {
    activeList() { return this.activeType === 'cos' ? this.allCosList : this.allSuitList },
    filteredList() { const search = String(this.searchKey || '').trim().toLowerCase(); if (!search) return this.activeList; return this.activeList.filter((item) => [item.fuzhuangmingcheng,item.fuzhuangkuanshi,item.mianliaoleibie,item.fuzhuangxiangqing].some((field) => String(field || '').toLowerCase().includes(search))) }
  },
  watch: { activeType() { this.updateRouteQuery() } },
  created() { this.activeType = this.$route.query.type === 'suit' ? 'suit' : 'cos'; this.keyword = this.$route.query.keyword || ''; this.searchKey = this.keyword; this.loadData() },
  methods: {
    itemKey(item) { return `${this.activeType}-${item.id || item.fuzhuangbianhao || item.fuzhuangmingcheng || 'item'}` },
    coverUrl(item) { const first = String(item.huawentuan || '').split(',')[0] || ''; return first ? (first.startsWith('http') ? first : `${this.baseUrl}${first}`) : '' },
    buildTags(item) { return [item.mianliaoleibie || this.texts.materialTag, `${this.texts.hotPrefix}${item.clicknum || 0}`] },
    applyFilters() { this.searchKey = this.keyword.trim(); this.updateRouteQuery() },
    updateRouteQuery() { const nextQuery = { type: this.activeType }; const keyword = this.searchKey.trim(); if (keyword) nextQuery.keyword = keyword; if (JSON.stringify(nextQuery) !== JSON.stringify(this.$route.query || {})) this.$router.replace({ path: '/index/browse', query: nextQuery }) },
    goDetail(item) { if (this.activeType === 'cos') { const target = { path: '/index/remaicosfuDetail', query: { detailObj: JSON.stringify(item) } }; if (!localStorage.getItem('Token')) { this.$message.info(this.texts.needLogin); this.$authDialogBus.$emit('open', { mode: 'login', role: 'yonghu', redirect: target }); return } this.$router.push(target); return } this.$message.info(this.texts.suitTip) },
    async loadData() { this.loading = true; try { const [cosRes, suitRes] = await Promise.all([ this.$http.get('remaicosfu/list', { params: { page: 1, limit: 100, sort: 'clicknum', order: 'desc' } }), this.$http.get('remaixifu/list', { params: { page: 1, limit: 100, sort: 'clicknum', order: 'desc' } }) ]); if (cosRes.data.code === 0) this.allCosList = (cosRes.data.data && cosRes.data.data.list) || []; if (suitRes.data.code === 0) this.allSuitList = (suitRes.data.data && suitRes.data.data.list) || [] } finally { this.loading = false } }
  }
}
</script>
